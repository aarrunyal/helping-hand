                        ...
                        @if($documents->count()>0)
                            @foreach($documents as $c=>$document)
                                <tr>
                                    ...
                                    <td class="text-center">{{((!empty($document->course)?ucwords($document->course->title):'-'))}}</td>
                                    <td class="text-center">{{ ucwords($document->access_type) }}</td>
                                    <td class="text-center">{!! getStatusLayout($document->is_active) !!}</td>
                                    <td class="text-center">
                                        <a href="{{route('document.edit', $document->id)}}"><i class="fas fa-edit"></i></a>
                                        <a href="{{route('document.destroy', $document->id)}}"><i class="fas fa-trash"></i></a>
                                    </td>
                                </tr>
                            @endforeach
                        @else
